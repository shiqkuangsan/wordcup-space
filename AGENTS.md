...
- 淘汰赛连续赛程默认使用“滚动两场窗口”：先联合评估第 `1/2` 场；第 1 场结束后立即更新赛果、注单腿和余额，再联合评估第 `2/3` 场；随后按同样方式推进第 `3/4` 场。不要在第一天一次性锁死后续全部比赛。
- 当前一场命中并留下在途串单时，下一场决策必须把该票视为现有资产：先计算不对冲基线、剩余中奖路径和潜在返还，再决定让票继续跑、做真正对冲，或用单关/新串关扩宽路径。扩宽路径不能包装成保本或稳定锁利。
