...
默认操作入口是当前聊天窗口，不是系统表单。你发截图、口述、平台成功页或赛果后，Codex 应代你调用本地脚本/API 完成 dry-run 和写入。

Codex 本地操作优先级：

| 场景 | Codex 应做 |
|---|---|
| 你发已成交注单截图/口述 | 解析成 placed-bet payload，先运行 `pnpm record:placed-bet` dry-run。 |
| 你确认“写入/确认/录进去” | 再运行 `pnpm record:placed-bet -- --write`，并回读注单号、余额和 legs 核对。 |
| dry-run 提示注单号已存在 | 不重复写入，只报告系统已有记录和冲突点。 |
| 信息缺关键字段 | 只追问缺失的最小信息，不要求你手动填表。 |

写库前最少需要：比赛或比赛文本、玩法、选择、金额、赔率、平台账户、是否真实资金、注单号或成功页证据。串关还需要每条 leg 的比赛、盘口、选择、line 和赔率。
...
如果截图是串关成功页，Codex 应按整张注单录入，不要拆成多张单关：

```text
我刚刚自己买了一场/一张，按截图录到 User，真实资金。
```

截图里若出现 `投注成功`、`注单号`、`2串1`、`总投注额`、`总可赢金额` 和多条比赛选择，Codex 应先抽取：
...
| 总投注额 | 父级 `stake` |
| 串关类型 | `mode: "parlay"` |
| 整张总赔率 | 父级 `finalOdds`；没有总赔率时用 legs 相乘 |
| 每条比赛/盘口/选择/line | `legs[]` |
| 每条 leg 赔率 | `legs[].finalOdds` |
| 可赢金额 | 用来核对；平台常显示盈利，系统 `potentialReturnCents` 记录含本金返还 |

示例解析：

```text
Betway 投注成功，User 真实资金，2串1。
加拿大 vs 波黑，上半场大小，小0.5 @ 2.53。
美国 vs 巴拉圭，上半场大小，小0.5 @ 2.44。
总投注 50，注单号 2606121327375036。先 dry-run，不要重复写入。
```

Codex 应优先用 `POST /api/placed-bets`：

- `dryRun: true` 先预览。
- 如果注单号已经存在，只提示重复风险，不再写入。
- 父级 slip 存总赔率和总 stake。
- `bet_slip_legs` 必须保留每条 leg 自己的赔率；不得把串关总赔率复制到所有 legs。
